        ...
        <main class="col-md-10 content">
            <div class="card">
                <div class="card-header">
                    <?php echo htmlspecialchars($_GET["table"]); ?>
                </div>
                <div class="card-body">
                    <!-- Main content -->
                                    ...
                                    if ($rows) {
                                        ...
                                        foreach ($rows as $row) {
                                            ...
                                            echo '<td>
                                                ...
                                                <a href="delete.php?table=' . htmlspecialchars($table_name) . '&id=' . $row[$primaryKey] . '" class="btn btn-sm btn-danger" onclick="return confirm(\'Apakah Anda yakin ingin menghapus data ini?\');"><i class="fas fa-trash-alt"></i>&nbsp;Delete</a>
                                            </td>';
                                            echo '</tr>';
                                        }
                                        echo '</tbody>';
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="insertForm" action="insert_data.php?table=<?php echo htmlspecialchars($table_name); ?>" method="post">
                    <?php
                    foreach ($columns as $column) {
                        if ($column != $primaryKey) { // Assuming primary key is an auto-increment column
                            echo '<div class="form-group">';
                            echo '<label for="' . htmlspecialchars($column) . '">' . ucfirst(htmlspecialchars($column)) . ':</label>';
                            echo '<input type="text" class="form-control" id="' . htmlspecialchars($column) . '" name="' . htmlspecialchars($column) . '" required>';
                            echo '</div>';
                        }
                    }
                    ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="editForm" action="edit_data.php?table=<?php echo htmlspecialchars($table_name); ?>" method="post">
                    ...
                    foreach ($columns as $column) {
                        echo '<div class="form-group">';
                        echo '<label for="edit_' . htmlspecialchars($column) . '">' . ucfirst(htmlspecialchars($column)) . ':</label>';
                        echo '<input type="text" class="form-control" id="edit_' . htmlspecialchars($column) . '" name="' . htmlspecialchars($column) . '" required>';
                        echo '</div>';
                    }
                    ?>
                    <input type="hidden" id="edit_<?php echo htmlspecialchars($primaryKey); ?>" name="<?php echo htmlspecialchars($primaryKey); ?>">
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Save changes</button>
                    </div>
            ...
            <div class="modal-body">
                ...
                <form id="viewForm">
                    ...
                    foreach ($columnNames as $column) {
                        echo '<div class="form-group">';
                        echo '<label for="view_' . htmlspecialchars($column) . '">' . ucfirst(htmlspecialchars($column)) . ':</label>';
                        echo '<input type="text" class="form-control" id="view_' . htmlspecialchars($column) . '" name="' . htmlspecialchars($column) . '" readonly>';
                        echo '</div>';
                    }
                    ?>
                    <div class="modal-footer">
