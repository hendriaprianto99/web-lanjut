...
echo "<form action=editdokumenexec.php method=post>";
$result = mysqli_query($con, "SELECT * FROM dokumen where No_Dokumen = '". mysqli_real_escape_string($con, $_GET['No_Dokumen']) . "'");
while($row = mysqli_fetch_array($result))
{
echo "<tr><td colspan=2><input type=hidden name=pk value='" . $row['No_Dokumen'] . "'></td></tr>";
...
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;No_Dokumen&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE><input type=text  class='form-control' name='No_Dokumen' value='". $row['No_Dokumen'] ."' size = 100></td>";
...
if ($r['Kode'] == $row['Kode']) {
echo "<option value='". $r[Kode] ."' selected>". $r[Kode] ." | ". $r[Kategori] ."</option>";
}else{
...
echo "<td bgcolor=CCEEEE><input type=text  class='form-control' name='Judul' value='". $row['Judul'] ."' size = 100></td>";
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Deskripsi&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE><textarea <textarea class='form-control' name='Deskripsi'>". $row['Deskripsi'] ."</textarea></td>";
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Tanggal_Pembuatan&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE><input type=text id='Tanggal_Pembuatan' name='Tanggal_Pembuatan' value='". $row['Tanggal_Pembuatan'] ."' size=10><script type='text/javascript'>calendar.set('Tanggal_Pembuatan');</script></td>";
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Tanggal_Modifikasi&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE><input type=text id='Tanggal_Modifikasi' name='Tanggal_Modifikasi' value='". $row['Tanggal_Modifikasi'] ."' size=10><script type='text/javascript'>calendar.set('Tanggal_Modifikasi');</script></td>";
