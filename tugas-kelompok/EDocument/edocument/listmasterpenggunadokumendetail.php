...
<?php
//cek otoritas
$q = "SELECT * FROM tw_hak_akses where tabel='pengguna/dokumen' and user = '". $_SESSION['Email'] ."' and detailData='1'";
...
echo "<tr><td><font face=Verdana color=black size=1>pengguna</font></td></tr>";
if(isset($_GET['Kode_Pengguna'])){$result = mysqli_query($con, "SELECT * FROM pengguna where Kode_Pengguna = '". mysqli_real_escape_string($con, $_GET['Kode_Pengguna']) . "'");}
...
echo "<td bgcolor=CCCCCC><font face=Verdana color=black size=1><b>Email</b></font></td>";
echo "<td bgcolor=CCCCCC><font face=Verdana color=black size=1><b>Telepon</b></font></td>";
...
echo "<tr><td align=left><a href=listmasterpenggunadokumen.php><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i>&nbsp;Back</font></button></a></td></tr>";
echo "</table>";
echo "</div>";
echo "<br><br>";
echo "<font face=Verdana color=black size=1>dokumen</font><br>";
if(isset($_GET['Kode_Pengguna'])){echo "<a href=insertmasterpenggunadokumendetail.php?Kode_Pengguna=". mysqli_real_escape_string($con, $_GET['Kode_Pengguna']) ."><button type='button' class='btn btn-light'><font face=Verdana size=1><i class='fa fa-plus'></i>&nbsp;Insert</font></button></a><br><br>";}
...
if(isset($_GET['Kode_Pengguna'])){$result = mysqli_query($con, "SELECT * FROM dokumen where dokumen.Kode_Pengguna= '".mysqli_real_escape_string($con, $_GET['Kode_Pengguna'])."' LIMIT $posisi,$batas");}
echo "<div class='table-responsive'> "; 
echo "<table class='table table-striped'>"; 
$firstColumn = 1;
$warna = 0;
...
$firstColumn = 0;
  ...
  if(isset($_GET['Kode_Pengguna'])){echo "<td><a class=linklist href=viewmasterpenggunadokumendetail.php?No_Dokumen=".$row['No_Dokumen']."&Kode_Pengguna=". mysqli_real_escape_string($con, $_GET['Kode_Pengguna']) ."><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i></font></button></a></td>";}
  if(isset($_GET['Kode_Pengguna'])){echo "<td><a class=linklist href=editmasterpenggunadokumendetail.php?No_Dokumen=".$row['No_Dokumen']."&Kode_Pengguna=". mysqli_real_escape_string($con, $_GET['Kode_Pengguna']) ."><button type='button' class='btn btn-primary'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a></td>";}
  if(isset($_GET['Kode_Pengguna'])){echo "<td><a class=linklist href=deletemasterpenggunadokumendetail.php?No_Dokumen=".$row['No_Dokumen']."&Kode_Pengguna=". mysqli_real_escape_string($con, $_GET['Kode_Pengguna']) ." onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger'><font face=Verdana size=1><i class='fa fa-Trash'></i></font></button></a></td>";}
...
if(isset($_GET['Kode_Pengguna'])){$tampil2 = mysqli_query($con, "SELECT * FROM dokumen where dokumen.Kode_Pengguna='".mysqli_real_escape_string($con, $_GET['Kode_Pengguna'])."'");}
